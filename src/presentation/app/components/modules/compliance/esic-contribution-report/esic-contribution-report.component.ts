...
import { ESICAttachment, EsicChallanList, PayrollESICList } from '../../../../../../domain/models/esic-challan.model';
import { ToasterService } from '../../../../../../common/toaster-service';


@Component({
  selector: 'app-esic-contribution-report',
  templateUrl: './esic-contribution-report.component.html',
  encapsulation: ViewEncapsulation.None,
  styleUrl: './esic-contribution-report.component.scss'
})
export class EsicContributionReportComponent implements OnInit {
  ...
  challanData = new EsicChallanList();
  isHistoryData: boolean = false;

  ngOnInit(): void {
    const navigationState = history.state;
    if (navigationState) {
      this.challanData = navigationState.EsicChallanData;
      if (this.challanData) {
  ...
  getReportData() {
    const params = new HttpParams()
      .set('FilterBy', 'name.contains("ESIC Report")')
  ...
  getChallanHistoryById(challanId: any) {
    ...
    const params = new HttpParams()
      ...
      .set('ESICChallanHistoryId', challanId)
      .set('PageNumber', this.pageNumber)
      .set('RecordCount', this.recordCount);
    this.pfChallanService.getPaginationDataNew<PayrollESICList[], ESICAttachment>(AppConstant.GET_CHALLAN_HISTORY_BY_ID_ESIC, params).subscribe({
      next: (response) => {
        if (response.success) {
  ...
  onGenerate() {
    ...
    if (this.pfContributionReportForm.valid) {
      ...
      if (this.pfContributionReportForm.value.selectedType === 2) {
        ...
        this.pfChallanService.getPaginationDataNew<PayrollESICList[], ESICAttachment>(AppConstant.GET_ESIC_CONTRIBUTION_REPORT, params).subscribe({
          next: (response) => {
            if (response.success) {
  ...
  downloadFile(row: any, type: string): void {
    ...
    let url = '';
    if (type === 'ECR File') {
      const attachment = this.attachments.find(a => a.payrollESICId === row.payrollESICId)?.attachments;
    ...
    } else if (type === 'ESIC Challan') {
      const attachment = this.attachments.find(a => a.payrollESICId === row.payrollESICId)?.attachments;
      attachment?.forEach(element => {
        if (element.fileType === 585) {
          url = element.filePath;
        }
      });
      window.open(url, '_blank');
